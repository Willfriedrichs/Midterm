...
```{r school data,warning=FALSE}
School_Facilities <- 
  st_read("https://opendata.arcgis.com/datasets/c08c48b2aadd4324ad39f8dca95e1755_0.geojson")%>%
  st_transform('ESRI:102254')%>%
  filter(TYPE == "Educational Facility")%>%
  st_centroid()

ggplot()+
  geom_sf(data = boulder.sf, fill = "grey70") +
  geom_sf(data = School_Facilities,colour = "green")
...
boulder <-
  boulder %>% 
    mutate(
      school_distance = nn_function(st_c(boulder), st_c(School_Facilities), 1))

st_drop_geometry(boulder) %>% 
  mutate(Age = 2021 - builtYear) %>%
  dplyr::select(price, mainfloorSF, Age, TotalFinishedSF,school_distance) %>%
...
School_PlayGroud <- st_read("Playground_Sites_Points.geojson")
```
