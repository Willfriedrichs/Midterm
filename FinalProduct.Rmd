...
<<<<<<< Updated upstream
This project involves using Geospatial machine learning to predict housing prices in Boulder County, Colorado.  The project can be categorized into four broad stages: data gathering, feature creation, multivariate regression modeling, and generating predictions.  Though these stages are presented as sequential, in development the process is cyclical, requiring periodic returns to earlier stages. Presented in this document is R code which can be adapted for other localities, along with plots and tables representing the content of the analysis.  
...
Along with written analysis, plots, and tables, the R code that produces the model is presented in the document in chunks. 
_The chunk below loads 17 libraries for use later, helps set up the generation of the r markdown file, gives access to online resources helpful to our analysis, and sets up a color scheme to be used for data visualization._
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
### Functions & Color Pallettes


```{r Functions & Color Pallettes, echo = FALSE}
=======
# set up knitr
...
We begin with  tract-level geography, population, and demographic data pulled from the US Government Census API.  The data uses American Community Survey results for 2019.  Variables of interest are isolated and wrangled to create features required for the analysis.
...
The primary data for the analysis was provided along with the project requirements.  Here we loaded, wrangled, and joined the data with the ACS geographies from above.  Likewise, Boulder County and City boundaries are established from open-source geometry files.  Table 1, below, summarizes key indicators by municipality. We can see that the price is very much depended on mucipality. 
