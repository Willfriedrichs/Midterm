...
ggplot(data = train.Data, aes(head_nn5, price)) +
  ...
  labs(title = "Relationship between Housing Price and the Average Distance\nto the Nearest Five Trail Heads", x = "Distance (m)", y = "Sale Price ($)",caption = "Fig. 2")+
  plotTheme()
```
...
cleanData %>%
  select(price,"Playground\nCount in 500m" = pgcount500m, 
         "Total Sq ft" = TotalFinishedSF,"Distance to\nLandmark (m)" = landmark_dist,
         "Distance to the\nnearest private\nschool" = privat_dist)%>%
...
# Run model on the 25-point test set
train.Data.2 <-
  train.Data.2 %>%
  mutate(SalePrice.Predict = predict(reg1, train.Data.2))
         
# Calculate ale price error, absolute error, and APE
train.Data.2 <-
  train.Data.2 %>%
...
```{r Predictions by municipality, echo=FALSE, warning=FALSE, message=FALSE}
...
# Clean training data for merge with test data
train.Data.2 <-
...
#  Plot predictions by municipality
ggplot()+
...
# Summarize mean absolute error, MAPE, and mean price
tracts_with_error <-
  tracts_with_error %>%
  group_by(GEOID.x)%>%
...
# Plot absolute Percentage Error as a function of Mean Housing Prices by Municipalities
ggplot(data = tracts_with_error, aes(Mean.price, MAPE.Pct)) +
  ...
  labs(x = "Mean Housing Prices",y = "Mean Absolute Percentage Errors", 
       title = "Absolute Percentage Error as a function of Mean\nHousing Prices by Municipalities",
...
# Select 8 home variables for InterCh.va
InterCh.va <-
studentData %>%
...
# Select 4 ACS variables for SpacialStruct.va
 SpacialStruct.va <-
   studentData %>%
...
# Select 6 additional geographic variables for Amen.va
 Amen.va <-
   studentData %>%
...
# Make Amen.va variables numeric for analysis
Private.Dist <-
...
# Bind and rename Amen.va variables
Amen.va <-
...
# Display InterCh.va variables via stargazer
...
# Display Amen.va variables via stargazer
...
# Display SpacialStruct.va variables via stargazer
